...
CuckooSearch_rand::CuckooSearch_rand(algorithmCategory cat, Utility& u, vector<Solution>& currentSolution, TerminationCriteria& t, double pa): Solver (cat, u, currentSolution) {
...
void CuckooSearch_rand::selectNestToLayACuckoo() {
	int selectedSolutionIndex = 0;	switch (selectionMethod)	{
...
bool CuckooSearch_rand::steps(){ // improveNewCuckoo
	...
	double fff = pertubedSolution.getSolutionFitness();

	neighborStructureType randomNeighorhood = static_cast<neighborStructureType>(mNeighbor->neighborStructure.at(Utility::getRandomUniformNoBetween(0, mNeighbor->neighborStructure.size())));

	neighborcount.at(randomNeighorhood).ne_count++;
	...
	if (mNeighbor->obtainedNeighbor.size() == 0) { /*cout << " local optima using this neighborhood: " << randomNeighorhood <<  ", using pert: " << levyFlightNo  << endl; */ return false;  };

	//cout << CurrentSolution->getSolutionFitness() << ", " << fff;

	neighborcount.at(randomNeighorhood).ne_count_succ++;
			...
			if (mNeighbor->neighborSelector == FIRST){
				...
				Solver<Solution>* localSearch = new GreatDeluge(SINGLE_BASED, localSearchseting, *CurrentSolution);
			...
			//cout << CurrentSolution->getSolutionFitness() << endl;
			//cout << ", " << pertubedSolution.getSolutionFitness() << ", "<< levyFlightNo << ", "<< t->best->getSolutionFitness() << endl;
	abandonWorseCuckoo();
	return false;
}

void CuckooSearch_rand::abandonWorseCuckoo(){
...
bool CuckooSearch_rand::createNewCuckoo(){
...
bool CuckooSearch_rand::acceptanceCriteria(double ImprovmentValue){
